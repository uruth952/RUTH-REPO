...
Graph3 <- Graph3 %>%
  mutate(
    ...
    share = n / total * 100,
    SEM = sqrt((share / 100) * (1 - share / 100) / total) * 100  
  ) %>%
  mutate(type = ifelse(elite == 0, "Undergraduate", "Elite"))

GraphC <- ggplot(Graph3, aes(x = levelk, y = share, fill = type)) +
  geom_bar(position = "dodge", stat = "identity") +
  geom_errorbar(aes(ymin = share - SEM, ymax = share + SEM), 
...
The null hypothesis of no difference in MAOs between elite and undergraduate students is rejected,this is  because it indicates a significantly higher averages among elite students. Conversely, with the alternative hypothesis, this  suggests a nonzero difference between the groups.
